...
export default function VariableBudgetsPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    budget_amount: '',
    description: '',
    budget_type: 'Variable' as BudgetType,
  });
  ...
  const loadBudgets = useCallback(async () => {
    try {
      ...
      const data = await masterBudgetService.getByType('Variable', true);
      setBudgets(data);
      ...
      console.error('Failed to load variable budgets:', err);
      toast.showToast(
        err instanceof Error ? err.message : 'Failed to load variable budgets',
        'error'
      );
    } finally {
      setIsLoading(false);
  ...
  const loadHistory = useCallback(async () => {
    try {
      ...
      setHistory(data.filter(entry => 
        entry.new_data?.budget_type === 'Variable' || entry.old_data?.budget_type === 'Variable'
      ));
    } catch (err) {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData({ name: '', budget_amount: '', description: '', budget_type: 'Variable' });
      setShowAddForm(false);
      await loadBudgets();
      await loadHistory();
      toast.showToast('Variable budget created successfully', 'success');
    } catch (err) {
      toast.showToast(
        err instanceof Error ? err.message : 'Failed to create variable budget',
        'error'
      );
    } finally {
      setSaving(false);
  ...
  const handleEdit = async (id: string, updates: { name?: string; budget_amount?: number; description?: string | null; budget_type?: BudgetType }) => {
    ...
    try {
      ...
      await loadBudgets();
      await loadHistory();
      toast.showToast('Variable budget updated successfully', 'success');
    } catch (err) {
      toast.showToast(
        err instanceof Error ? err.message : 'Failed to update variable budget',
        'error'
      );
    } finally {
      setSaving(false);
  ...
  const handleDelete = async (id: string) => {
    confirmDialog.showConfirm({
      title: 'Delete Variable Budget',
      message: 'Are you sure you want to delete this variable budget? This will not affect existing monthly budgets, but new months will not include this category.',
      confirmText: 'Delete',
      cancelText: 'Cancel',
      variant: 'danger',
      onConfirm: async () => {
        ...
        try {
          ...
          await loadBudgets();
          await loadHistory();
          toast.showToast('Variable budget deleted successfully', 'success');
        } catch (err) {
          toast.showToast(
            err instanceof Error ? err.message : 'Failed to delete variable budget',
            'error'
          );
        } finally {
          setSaving(false);
  ...
  const variableChartData: PieChartData[] = budgets.map(b => ({
    name: b.name,
    ...
    <div className="max-w-4xl mx-auto space-y-6 animate-fade-in">
      ...
      {budgets.length > 0 && (
        <Card variant="raised" padding="lg">
          <h3 className="text-headline text-[var(--color-text)] mb-4">Variable Budgets Breakdown</h3>
          <PieChart
      ...
      {budgets.length === 0 ? (
        <Card variant="raised" padding="lg">
          <p className="text-body text-[var(--color-text-muted)] text-center py-8">
            No variable budgets yet. Click "Add Variable Budget" to create your first one.
          </p>
        </Card>
      ) : (
      ...
      <Card variant="raised" padding="lg">
        ...
        <p className="text-small text-[var(--color-text-muted)] mb-4">
          Recent changes to variable budgets.
        </p>
        {historyLoading ? (
